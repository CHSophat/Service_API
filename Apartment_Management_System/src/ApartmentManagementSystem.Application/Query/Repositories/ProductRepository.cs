...
namespace ApartmentManagementSystem.Application.Query.Repositories
{
    public class ProductRepository : IProductRepository
    {
        ...
        {
            return await _context.Products
                .Where(p => p.Status == "available" || p.Status == "vacant")
                .Include(p => p.ProductPhotos)
                .Skip((pageNumber - 1) * pageSize)
                .Take(pageSize)
                .ToListAsync();
        ...
        {
            return await _context.Products.Where(p => p.Status == status).CountAsync();
        }
        ...
        {
            var query = _context.MaintenanceRequests.Where(mr => mr.ProductId == productId);
            ...
            return await query.OrderByDescending(mr => mr.CreatedAt).ToListAsync();
        }
        ...
        {
            return await _context.MaintenanceRequests
                .Where(mr => mr.Status == "open" || mr.Status == "in-progress")
        ...
        {
            return await _context.Products.Where(p => p.ProductType == productType).CountAsync();
        }
